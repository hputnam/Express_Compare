...
#gene count matrix
gcount <- as.data.frame(read.csv("/Users/laurenzane/Desktop/Putnam_Lab/Projects/Express_Compare/RAnalysis/Data/Pacuta/PAcuta_gene_count_matrix.csv", row.names="gene_id")) # pacuta
...
# subset for PolyA and RiboFree samples to compare differences in gene expression due to library prep method (remove rows with zeros and see what is left)

PolyA.subset <- gcount_filt %>% select(1:3)

PolyA.no.zeros <- filter(PolyA.subset, X1041.gtf > "0", X1471.gtf > "0", X1637.gtf  > "0")
#19941                         
                         
  
RiboFree.subset <- gcount_filt %>% select(4:6)

RiboFree.no.zeros <- filter(RiboFree.subset, Sample1.gtf > "0", Sample2.gtf > "0", Sample3.gtf > "0" )
#20463
