...
## Mcap symbiont 
sym_1101 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/symBLAST_results/1101.1_sym_blast_results.txt", sep = "\t", header = FALSE)
colnames(sym_1101) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
sym_Sample4 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/symBLAST_results/Sample4.1_sym_blast_results.txt", sep = "\t", header = FALSE)
...
```{r }
## Pacuta symbiont
sym_1041 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/symBLAST_results/1041.1_sym_blast_results.txt", sep = "\t", header = FALSE)
colnames(sym_1041) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
sym_Sample1 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/symBLAST_results/Sample1.1_sym_blast_results.txt", sep = "\t", header = FALSE)
...
## Mcap bacteria
bac_1101 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/bacBLAST_results/1101_bac_blast_results.txt", sep = "\t", header = FALSE)
colnames(bac_1101) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
bac_Sample4 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/bacBLAST_results/Sample4_bac_blast_results.txt", sep = "\t", header = FALSE)
...
```{r }
## Pacuta bacteria
bac_1041 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/bacBLAST_results/1041.1_bacteria_blast_results.txt", sep = "\t", header = FALSE)
colnames(bac_1041) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
bac_Sample1 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/bacBLAST_results/Sample1_bac_blast_results.txt", sep = "\t", header = FALSE)
...
## Mcap virus
virus_1101 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/virusBLAST_results/1101_virus_blast_results.txt", sep = "\t", header = FALSE)
colnames(virus_1101) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
virus_Sample4 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/virusBLAST_results/Sample4_virus_blast_results.txt", sep = "\t", header = FALSE)
...
```{r }
## Pacuta virus
virus_1041 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/virusBLAST_results/1041_virus_blast_results.txt", sep = "\t", header = FALSE)
colnames(virus_1041) <- c("qseqid","sseqid", "pident", "length", "mismatch", "gapopen", "qstart", "qend", "sstart", "send", "evalue", "bitscore")
virus_Sample1 <- read.delim("/Users/laurenzane/Desktop/Putnam_Lab/BLAST_results/virusBLAST_results/Sample1_virus_blast_results.txt", sep = "\t", header = FALSE)
...
## Mcap 
bac_virus <- intersect(bac_Sample4$qseqid, virus_Sample4$qseqid)# blast queries common to bac/virus
length(bac_virus) 
c(bac_virus) #creating a list from intersecting queries
bac_virus_df <- subset(bac_1041, bac_1041$qseqid %in% bac_virus) #using subset to create
# a dataframe with bac/virus intersecting queries
dim(bac_virus_subset) # dimensions of dataframe

# virus_bac_subset <- subset(virus_Sample1, virus_Sample1$qseqid %in% bac_virus)
bac_sym <- intersect(bac_Sample4$qseqid, sym_Sample4$qseqid)
length(bac_sym) 
c(bac_sym)
bac_sym_df <- subset(bac_1041, bac_1041$qseqid %in% bac_sym)

virus_sym <- intersect(sym_Sample4$qseqid, virus_Sample4$qseqid)
length(virus_sym) 
c(mcap_virus_sym)
virus_sym_df <- subset(virus_1041, virus_1041$qseqid %in% virus_sym)


```

## intersection of all three taxa 
```{r}
all_intersect <- intersect(bac_sym, virus_Sample4$qseqid)
length(all_intersect) 

all_subset_1101 <- subset(sym_1101, sym_1101$qseqid %in% mcap_polya_all)
all_subset_1101 %>% count(qseqid) 
```
## work with the bacteria + virus intersection dataframe to assign reads to bacteria BLAST database 
```{r}
max(bac_virus_df$evalue)
min(bac_virus_df$evalue)
hist(bac_virus_df$evalue)
