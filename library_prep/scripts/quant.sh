...
do
	...
	then
		...
		kallisto quant -i "$base"/indices/mcap_index.idx -o "$base"/output/mcap/"${ids[$i]}" "${ids[$i]}"_1P "${ids[$i]}"_2P
	else
		#echo $fileNum is P. acuta	
		kallisto quant -i "$base"/indices/pacuta_index.idx -o "$base"/output/pacuta/"${ids[$i]}" "${ids[$i]}"_1P "${ids[$i]}"_2P
	fi	
done

#updates the permissions and nesting of the files such that the path to 
#output is /r/corals/output/[mcap/pacuta]/[sample_id]/kallisto

#directories in output are mcap and pacuta
for dir in "$base"/output/*
do
	chmod 775 $dir
	cd $dir
