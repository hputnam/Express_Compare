...
for ID in ${ids[@]}; do
      	stringtie -p $SLURM_NTASKS_PER_NODE -e --conservative -C "$out"/"$ID"_coverage.gtf -G ref/Structural_annotation_experimental.gff -o "$out"/"$ID".gtf bam/trimmomatic/paired/"$ID".bam
